...
Either way, cd into the resultant folder on your machine and run 'rake' on the command line - you should get some failing tests followed by a character count.  The tests are all listed in spec/golf_spec.rb, and they all require you to write class methods in the Golf class which is in lib/golf.rb.
